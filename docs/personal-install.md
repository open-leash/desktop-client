...
Download the current macOS or Windows artifact from the Leash website or the matching `desktop-client` GitHub release. The release feed verifies platform, architecture, SHA-256, rollout status, and version before offering an update.
...
Docker is used only for Postgres. Built-in Features run directly in `client-api` and require no Feature images, Docker socket, sandbox pool, or marketplace.
