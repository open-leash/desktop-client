...
PACKAGED_LOCAL_CLOUD_USER_DATA = (
    ROOT / "apps" / "desktop" / ".dev" / "packaged-local-cloud"
)
...
def build_modes() -> dict[str, Mode]:
    ...
    personal_api = Command("engine", ["npx", "tsx", "src/server.ts"], {
        **base, "OPENLEASH_API_PORT": "9318", "OPENLEASH_API_SURFACE": "client",
        "OPENLEASH_DEPLOYMENT_MODE": "individual-open-source",
    }, ROOT / "apps" / "engine")
    personal_desktop = Command("desktop-client", ["npx", "electron", "apps/desktop", "--show-window"], {
    ...
    cloud_api = Command("engine", ["npx", "tsx", "src/server.ts"], {
        **base, "OPENLEASH_API_PORT": "9318", "OPENLEASH_API_SURFACE": "client",
    ...
    cloud_desktop = Command("desktop-client", ["npx", "electron", "apps/desktop", "--show-window"], {
    ...
    main_web = Command("main-web", ["npm", "run", "dev"], {
    ...
    return {
        "individual-open-source": Mode(
            ...
            "Local Leash Engine, Postgres, desktop client, flow viewer, and in-process Features.",
            (flow, personal_api, personal_desktop),
            (("Engine", "http://127.0.0.1:9318/health"), ("Flow viewer", "http://127.0.0.1:9340/healthz")),
        ),
        "public-cloud": Mode(
            "public-cloud", "Leash Cloud development",
            "Personal hosted-flow simulation with Leash Engine, the private website checkout, desktop, and no dashboard.",
            (flow, cloud_api, main_web, cloud_desktop),
            (("Engine", "http://127.0.0.1:9318/health"), ("Website", "http://127.0.0.1:9305"), ("Flow viewer", "http://127.0.0.1:9340/healthz")),
        ),
    }
...
def build_packaged_local_cloud_mode(dev_auth: bool = False) -> Mode:
    ...
    return Mode(
        ...
        "Release desktop connected to the local Cloud Engine, private website, Postgres, and flow viewer.",
        tuple(processes),
        development.urls,
    )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--plugins-dir", default=str(ROOT / "apps" / "engine" / "src" / "plugins"))
...
def startup_commands(
    ...
    if not desktop_only:
        commands.extend([
            ...
            Command("engine-build", ["npm", "run", "build", "-w", "@openleash/client-api"]),
            Command("migrate", ["npm", "run", "db:migrate", "-w", "@openleash/client-api", "--", "--apply"], {"DATABASE_URL": DATABASE_URL}),
            Command("bootstrap-personal", ["node", "apps/engine/dist/bootstrap-personal.js", "--name", "Personal Leash", "--slug", "individual-open-source", "--mode", "private"], {"DATABASE_URL": DATABASE_URL}),
            Command("verify-features", ["npx", "tsx", "--test", "src/plugins/feature-runtime.test.ts"], None, ROOT / "apps" / "engine"),
        ])
    if build_desktop:
...
def local_state_paths() -> tuple[Path, ...]:
    ...
    fixed = {
        ...
        ROOT / "apps" / "desktop" / ".dev" / "Leash.app",
        ROOT / "apps" / "desktop" / ".dev" / "OpenLeash.app",
        PACKAGED_LOCAL_CLOUD_USER_DATA,
